...
const AUTHOR_GUIDE: &str = "Choose the cheapest reliable protection first. When WT is useful, state exactly what the detector recognizes in rule.md. Submit JSON with documentation.source, code.source, raw-positive and raw-negative fixtures. New/update validate, format and test before storage. An acceptable review signal is still a raw-positive fixture; do not narrow a detector merely to make it disappear. Run wt check to inspect actual scope and findings. Optional intent: watch (default detect) marks a rule whose purpose is forcing re-review whenever specific code changes, not finding new occurrences; findings and repeated acceptances are its expected steady state, and wt stats reports it as watch instead of noisy. Pick the capability by what is being checked, not by habit: use ast.v1 for a rule about code in a supported language (python, javascript, typescript, tsx, rust) — a plain function-call or structural check in text.v1 will also match inside comments and string literals, and breaks on reformatting; use ast_match_context for a node kind that cannot stand alone as a bare pattern (a match arm, struct field, attribute, function parameter) and span::contains for an inside/not-inside relationship between two independently captured spans. Use toml.v1 for TOML manifests/config instead of a line-by-line regex. Reserve text.v1 for prose, comments, string-level checks, and file types neither ast.v1 nor toml.v1 covers (YAML, JSON, shell, Markdown, license headers, commented-out code). wt validate prints a non-blocking hint when a rule looks like it picked the wrong one.";
...
const LANGUAGE_GUIDE: &str = "wt-rule-1 is a restricted top-level statement body. Use declared static pattern names with rx::find_all(file, \"pattern\") and emit(matched.span, \"diagnostic\"); text.v1, ast.v1 and toml.v1 are explicit capabilities; file.path (and path::matches) specifically needs text.v1 or path.v1, so an ast.v1-only or toml.v1-only rule that reads it fails WT104. ast.v1 adds file.ast_match(language, pattern) for structural matches, m.node(\"NAME\") for a captured metavariable, and m.ast_match(language, pattern) to search inside a match; file.ast_match_context(language, context, selector) (also on a match) matches a contextual pattern instead, for a node kind that cannot stand alone as a bare pattern (a match arm, struct field, attribute, function parameter): context is a standalone snippet and selector is the node kind to extract from it; a child the context leaves out is unconstrained, not required absent, so a bare context also matches a more specific form (e.g. a guarded match arm) unless a second, narrower contextual pattern excludes it via span::contains. span::contains(outer, inner) is a boolean span-containment check for expressing \"X unless some Y's span encloses it\", independent of any capability. A file with any parse error node is an analysis gap, never a silent no-match. toml.v1 adds file.toml() for a structured TOML document: v.get(\"key\") for an optional child value, v.entries()/v.items() for a table's entries or an array's elements, and v.kind/v.text/v.span; a file that does not parse cleanly as TOML is an analysis gap, same as ast.v1. Only finite WT sequences can be iterated; detector programs cannot access the filesystem, external commands, or review state. Use wt plan for query inspection.";
...
fn capabilities() -> Result<Value> {
    Ok(
        ...
        "language":"wt-rule-1", "helpers":["text.v1","ast.v1","toml.v1"],
        ...
        "toml":{"parser":wt_runtime::TOML_PARSER_CRATE},
