...
const LANGUAGE_GUIDE: &str = "wt-rule-1 is a restricted top-level statement body. Use declared static pattern names with rx::find_all(file, \"pattern\") and emit(matched.span, \"diagnostic\"); text.v1, ast.v1 and toml.v1 are explicit capabilities. ast.v1 adds file.ast_match(language, pattern) for structural matches, m.node(\"NAME\") for a captured metavariable, and m.ast_match(language, pattern) to search inside a match; file.ast_match_context(language, context, selector) (also on a match) matches a contextual pattern instead, for a node kind that cannot stand alone as a bare pattern (a match arm, struct field, attribute, function parameter): context is a standalone snippet and selector is the node kind to extract from it. span::contains(outer, inner) is a boolean span-containment check for expressing \"X unless some Y's span encloses it\", independent of any capability. A file with any parse error node is an analysis gap, never a silent no-match. toml.v1 adds file.toml() for a structured TOML document: v.get(\"key\") for an optional child value, v.entries()/v.items() for a table's entries or an array's elements, and v.kind/v.text/v.span; a file that does not parse cleanly as TOML is an analysis gap, same as ast.v1. Only finite WT sequences can be iterated; detector programs cannot access the filesystem, external commands, or review state. Use wt plan for query inspection.";
