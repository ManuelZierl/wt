...
### 6.6 Structural and structured-data matching (`ast.v1`, `toml.v1`)
...
A rule must explicitly request the versioned `toml.v1` capability for structured access to a TOML document (a Cargo manifest, or other TOML config) instead of a line-by-line regex over its text:

- `file.toml()` parses the whole file and returns the document root value. A file that does not parse cleanly as TOML — including a TOML datetime literal, which the installed parser does not support (see `wt capabilities` for its identity) — is an analysis gap for the file, exit `2`, same contract as `ast.v1`'s parse-error-node rule, never a silent empty document.
- `value.get("key")` returns the direct child of a table value named `key`, or unit `()` when this is not a table or has no such key. It is not recursive: a dotted path is expressed by chaining `.get(...)` calls.
- `value.entries()` returns a finite sequence of a table's entries in source order; each entry has `.key` (text), `.key_span`, and `.value` (another document value). `value.items()` returns a finite sequence of an array's elements. Both return an empty sequence for a value that is not the matching kind.
- `value.kind` is one of `"table"`, `"array"`, `"string"`, `"integer"`, `"float"`, or `"boolean"`. `value.text` is the decoded string content for a `string` value, or the raw source text at `value.span` for every other kind — so an entry's key span or a value's span can each back a finding, and a renamed dependency (`alias = { package = "forbidden-crate", ... }`) is resolved by reading its `package` sub-key's value rather than trusting the entry's own key.
- A dotted-key assignment (`a.b.c = 1`) and the equivalent inline table (`a = { b = { c = 1 } }`) parse to the same table-of-tables shape; a rule does not need to special-case either form.
...
- [`crate-dependency-boundary/`](../examples/crate-dependency-boundary/) — a `toml.v1` rule enforcing that a Cargo manifest depends only on one workspace crate, across `[dependencies]`, `[dev-dependencies]`, `[build-dependencies]`, and a target-qualified table, resolving a renamed (`package = "..."`) dependency and a dotted-key table to the same shape a plain inline table gets, with a malformed-manifest analysis-gap case exercised directly against `wt-runtime` (`tests.json` cannot express an expected gap).
